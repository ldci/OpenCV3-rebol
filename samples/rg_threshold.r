...
sumRGB: func [src dst] [
	...
	r: cvCreateImage srcS/width srcs/height IPL_DEPTH_8U 1 
	g: cvCreateImage srcS/width srcS/height IPL_DEPTH_8U 1 
	b: cvCreateImage srcS/width srcS/height IPL_DEPTH_8U 1 
	...
	;Temporary storage.
	s: cvCreateImage srcS/width srcS/height IPL_DEPTH_8U 1 
...
loadImage: does [
	...
	if not none? temp [
		...
		if error? try [
			...
			dst: cvCreateImage srcS/width srcS/height srcS/depth 1
...
mainWin: [
	...
	text 30 options [info] "Threshold Type"
	
	tType: drop-list 45 "CV_THRESH_BINARY" data tType [threshold_type:  get to word! face/text makeThresh]
	pad 30
	button 25 "Quit" [if isFile [free-mem src free-mem dst ] freeDylib quit]
	return
	panel data [
